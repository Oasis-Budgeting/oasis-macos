...
## 2026-03-08 - [UI State Rehydration of Sensitive Data]
**Vulnerability:** A sensitive authentication token was needlessly loaded into a SwiftUI `@State` variable (`formToken`) upon view appearance (`.onAppear`), retaining credentials in memory and increasing exposure risk.
**Learning:** Rehydrating sensitive state variables (like `formToken`) strictly for initial form loading creates unnecessary memory exposure, as tokens should be managed securely by the backend state layer or Keychain and should only briefly exist in memory when explicitly provided by the user.
**Prevention:** Omit sensitive tokens from UI rehydration hooks like `.onAppear` to ensure they are handled as write-only inputs during authentication, reducing the window of credential exposure. Ensure subsequent logic verifies that input state variables are not empty before writing changes.
