...
## 2026-03-11 - [Insecure UI State Rehydration for Sensitive Tokens]
**Vulnerability:** The stored authentication token `storedAuthToken` was being automatically assigned to the UI state variable `formToken` during the `.onAppear` lifecycle method of the main view. This unnecessarily exposed the active, sensitive token in memory and potentially in the UI when not strictly needed for the initial render or subsequent operations.
**Learning:** Rehydrating sensitive credentials from secure storage back into generic UI state variables (`@State`) when a view appears defeats the purpose of clearing them post-use and increases the window of exposure for memory-based attacks or accidental UI leaks.
**Prevention:** Only retrieve and bind sensitive credentials to UI state variables when explicitly required for a user action (e.g., when the user is actively editing a connection profile). Ensure these state variables are explicitly cleared (`defer { formToken = "" }`) immediately after the action completes, and purposefully omit them from automatic UI rehydration logic like `.onAppear`.
